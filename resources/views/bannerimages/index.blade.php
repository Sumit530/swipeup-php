...
@section('content')
            ...
            <div class="content-header row">
                <div class="content-header-left col-md-9 col-12 mb-2">
                    <div class="row breadcrumbs-top">
                        <div class="col-12">
                            <h2 class="content-header-title float-start mb-0">Banner Images</h2>
                            <div class="breadcrumb-wrapper">
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Dashboard</a>
                                    </li>
                                    <li class="breadcrumb-item active">banner images
                                    </li>
                                </ol>
                            </div>
                        </div>
                ...
                <div class="content-header-right text-md-end col-md-3 col-12 d-md-block d-none">
                    <div class="mb-1 breadcrumb-right">
                        <div class="dropdown">
                            <a href="{{ url('admin/bannerimages/create') }}" class="btn-icon btn btn-primary btn-round btn-sm" type="button" title="" data-bs-toggle="tooltip" data-bs-placement="top" title="" data-bs-original-title="Add new language">
                                    ...
                                    <tbody>
                                        @if(!empty($bannerimages))
                                            @foreach($bannerimages as $k => $bannerimage)
                                                @php
                                                // song banner image
                                                if ($bannerimage->image_name != '') 
                                                {
                                                    $destinationPath =  Storage::disk('public')->path('uploads/banner_image');
                                                    if(!empty($bannerimage->image_name)){
                                                        if(File::exists($destinationPath.'/'.$bannerimage->image_name)) {
                                                            $banner_image = url('storage/app/public/uploads/banner_image/'.$bannerimage->image_name);
                                                        }
                                                        else
                                                        {
                                                            $banner_image = "";
                                            ...
                                            <tr>
                                                <td>{{ $k+1 }}</td>
                                                <td><a href="{{ $banner_image }}" target="_blank">{{ $bannerimage->image_name }}</a></td>
                                                <td>
                                                ...
                                                <td>
                                                    <div class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-end" style="">
                                                            @if($bannerimage->status == 1)
                                                             <a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#status-model" data-id="{{ $bannerimage->id }}">
                                                            ...
                                                            @endif
                                                            <a class="dropdown-item" href="{{ route('bannerimages.edit',array($bannerimage->id)) }}">
                                                                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit-2 me-50"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg>
                                                                <span>Edit</span>
                                                            </a>
                                                            <a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#delete-model" data-id="{{ $bannerimage->id }}">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="myModalLabel1">Change status banner image</h4>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form class="form" id="status_model" action="{{ route('admin.bannerimages-status') }}" method="POST">
                @csrf
                <div class="modal-body">
                    <p>
                        Are you sure want to change this banner image status..?
                    </p>
                </div>
    ...
    <div class="modal fade text-start" id="delete-model" tabindex="-1" aria-labelledby="myModalLabel1" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="myModalLabel1">Delete banner image</h4>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form class="form" id="delete_model" action="{{ route('admin.bannerimages-delete') }}" method="POST">
                @csrf
                <div class="modal-body">
                    <p>
